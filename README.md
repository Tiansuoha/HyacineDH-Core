...
```bash
sed -i 's@^\(deb.*stable main\)$@#\1\ndeb https://mirrors.cernet.edu.cn/termux/apt/termux-main stable main@' $PREFIX/etc/apt/sources.list && apt update && apt upgrade -y && apt install -y curl && curl -L -o run.sh https://gh-proxy.org/raw.githubusercontent.com/Tiansuoha/HyacineDH-Core/main/run.sh && chmod +x run.sh && ./run.sh
```
